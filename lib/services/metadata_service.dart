...
''';
  ...
  /// Crée un backup du gamelist.xml :
  /// /userdata/roms/<system>/backup_gamelist.xml (écrasé à chaque sauvegarde)
  /// La date du backup est ajoutée comme commentaire au début du fichier.
  Future<bool> backupGamelist(String systemName) async {
    ...
    // Le script Python : copie le fichier + insère un commentaire de backup
    final pyScript = '''
import sys, re
...
try:
    ...
    print('NO:' + str(e))
''';
    // Encode le script en base64 pour éviter tous les problèmes de quoting
  ...
  Future<bool> writeMetadata(
    ...
    // Encode tagsJson en base64 et l'embarque dans le script pour éviter tout
    // problème de quoting shell (apostrophes, parenthèses, args longs…).
