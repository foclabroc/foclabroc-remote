              ...
              child: Column(children: [
                  ...
                  child: Row(children: [
                    ...
                    if (_bestScore > 0)
                      Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                        ...
                        const Text('meilleur niveau', style: TextStyle(color: Colors.white38, fontSize: 10)),
                      ]),
                  ]),
                ),
                const SizedBox(height: 16),
                if (_bestScore > 0) ...[
                  Row(children: [
                    ...
                    _StatCard(icon: Icons.layers_rounded, color: Colors.cyanAccent, label: 'Niveau', value: '$_bestLevel'),
                    const SizedBox(width: 10),
                    _StatCard(icon: Icons.grid_view_rounded, color: Colors.purpleAccent, label: 'Briques', value: '${_brickCols * _brickRows * _bestLevel}+'),
                  ]),
                  const SizedBox(height: 16),
                ],
                // Règles
                Container(
                  ...
                  child: Column(children: [
                    ...
                    const SizedBox(height: 10),
                    _RuleRow(icon: Icons.star_rounded, color: Colors.amberAccent, text: '10 pts par brique + bonus combo 🔥'),
                    const SizedBox(height: 10),
                    _RuleRow(icon: Icons.speed_rounded, color: Colors.orangeAccent, text: 'Vitesse et lignes augmentent à chaque niveau'),
                    const SizedBox(height: 10),
                    _RuleRow(icon: Icons.shield_rounded, color: Colors.deepOrangeAccent, text: 'Briques résistantes (2 coups) dès le niveau 2'),
                    const SizedBox(height: 10),
                    _RuleRow(icon: Icons.expand_rounded, color: Colors.greenAccent, text: 'Batocera 🟢 → agrandit la raquette (+30px)'),
                    const SizedBox(height: 10),
                    _RuleRow(icon: Icons.compress_rounded, color: Colors.redAccent, text: 'Recalbox 🔴 → réduit la raquette ET −500 pts si attrapé !'),
                    const SizedBox(height: 10),
                    _RuleRow(icon: Icons.snooze_rounded, color: Colors.cyanAccent, text: 'Balle lente 🐢 → ralentit la balle 5 secondes'),
                    const SizedBox(height: 10),
                ...
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    label: const Text('Nouvelle partie', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
...
class _BreakoutGameState extends State<_BreakoutGame> with TickerProviderStateMixin {
  ...
  Future<void> _shareScore() async {
    try {
      ...
      await Share.shareXFiles(
        [XFile(file.path)],
        text: 'J\'ai fait $_score pts au Breakout Rétro ! 🎮🏆',
      );
    } catch (e) {
  ...
  Widget _buildGameOver() {
    ...
    final msg   = _lives <= 0 ? 'Game Over !' : _level > 3 ? 'Impressionnant !' : 'Niveau $_level terminé !';
        ...
        child: SingleChildScrollView(
          ...
          child: Column(children: [
            ...
            Row(children: [
              ...
              _StatCard(icon: Icons.layers_rounded, color: Colors.cyanAccent, label: 'Niveau', value: '$_level'),
              const SizedBox(width: 8),
              _StatCard(icon: Icons.favorite_rounded, color: Colors.redAccent, label: 'Vies', value: '$_lives/3'),
            ]),
            const SizedBox(height: 10),
            Row(children: [
              _StatCard(icon: Icons.grid_view_rounded, color: Colors.purpleAccent, label: 'Briques', value: '$_bricksDestroyed'),
              const SizedBox(width: 8),
              _StatCard(icon: Icons.bolt_rounded, color: Colors.orangeAccent, label: 'Power-ups', value: '$_powerUpsCollected'),
              const SizedBox(width: 8),
              _StatCard(icon: Icons.local_fire_department_rounded, color: Colors.deepOrangeAccent, label: 'Combo max', value: '×$_combo'),
            ]),
                ]),
              ),
            ),
