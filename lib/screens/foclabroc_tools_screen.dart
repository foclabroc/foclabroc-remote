              ...
              child: ListView(
                ...
                children: [
                  ...
                  _FoclabrocToolCard(
                    ...
                    title: 'YouTube TV',
                    subtitle: 'Installe YouTube TV dans Ports\n(Batocera x86_64)',
                  ...
                  _FoclabrocToolCard(
                    ...
                    subtitle: 'Installe la Toolbox dans Ports\npour y accéder depuis Batocera',
                  ...
                  _FoclabrocToolCard(
                    ...
                    title: 'RGSX',
                    subtitle: 'Installe le pack RetroGameSets\ndans Ports',
...
class _YoutubeTvInstallScreenState extends State<_YoutubeTvInstallScreen> {
  ...
  Future<void> _launch() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Row(children: [
          ...
          SizedBox(width: 10),
          Flexible(child: Text('Installer YouTube TV ?', overflow: TextOverflow.ellipsis)),
        ]),
        content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text(
            'Installe YouTube TV dans le menu Ports de Batocera.',
            style: TextStyle(color: Colors.white70, fontSize: 12, height: 1.5),
          ),
          const SizedBox(height: 10),
          Container(
    ...
    _appendLog('🗑️ Suppression ancienne installation...');
    ...
    _appendLog('\n⬇️ Téléchargement de YouTube TV...');
    ...
    _appendLog("✅ Téléchargement terminé (${(szCheck / 1024 / 1024).toStringAsFixed(0)} Mo).");
    ...
    // 5. Fichiers .dep
    _appendLog('⬇️ Téléchargement des dépendances...');
    ...
    _appendLog('\n✅ YouTube TV installé avec succès !');
    setState(() => _running = false);
      ...
      await showDialog(
        ...
        builder: (ctx) => AlertDialog(
          ...
          title: const Row(children: [
            Icon(Icons.check_circle_rounded, color: Color(0xFF50FA7B), size: 22),
            SizedBox(width: 10),
            Flexible(child: Text('YouTube TV installé !')),
          ]),
          content: const Text(
            'YouTube TV est disponible dans le menu Ports.',
            style: TextStyle(color: Colors.white70, fontSize: 12),
          ),
          actions: [
            ElevatedButton(onPressed: () => Navigator.pop(ctx), child: const Text('OK')),
                ...
                child: Padding(
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    const Row(children: [
                      ...
                      SizedBox(width: 8),
                      Text('YouTube TV pour Batocera',
                          style: TextStyle(color: Colors.redAccent, fontWeight: FontWeight.w700, fontSize: 15)),
                    ]),
                    const SizedBox(height: 10),
                    const Text(
                      'Installe YouTube TV dans le menu Ports. Accédez à des millions de vidéos directement depuis votre Batocera.',
                      style: TextStyle(color: Colors.white54, fontSize: 12, height: 1.5),
                    ),
                    const SizedBox(height: 8),
                    Wrap(spacing: 6, runSpacing: 6, children: [
                    ...
                    Container(
                      ...
                      child: const Row(children: [
                        Icon(Icons.warning_amber_rounded, color: Colors.redAccent, size: 14),
                        SizedBox(width: 6),
                        Text('Batocera x86_64 uniquement',
                            style: TextStyle(color: Colors.redAccent, fontSize: 11)),
                      ]),
                    ),
                    const SizedBox(height: 20),
                    SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        ...
                        icon: _running
                            ...
                            : const Icon(Icons.download_rounded),
                        label: Text(_running ? 'Installation en cours...' : 'Installer YouTube TV'),
                      ),
                    ),
                  ]),
                ),
...
class _FoclabrocToolboxInstallScreenState extends State<_FoclabrocToolboxInstallScreen> {
  ...
  Future<void> _launch() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Row(children: [
          ...
          SizedBox(width: 10),
          Flexible(child: Text('Installer dans Ports ?', overflow: TextOverflow.ellipsis)),
        ]),
        content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text(
            'Installe Foclabroc Toolbox dans le menu Ports de Batocera.',
            style: TextStyle(color: Colors.white70, fontSize: 12, height: 1.5),
          ),
          const SizedBox(height: 10),
          Container(
            ...
            child: const Row(children: [
              ...
              SizedBox(width: 6),
              Expanded(child: Text(
                'Relance EmulationStation à la fin.',
    ...
    _appendLog('🔍 Vérification de l\'architecture...');
    final arch = await _exec('uname -m');
    if (arch.trim() != 'x86_64') {
      _appendLog('❌ Architecture non supportée : $arch (x86_64 requis).');
      setState(() => _running = false);
      return;
    }
    _appendLog('✅ Architecture x86_64 détectée.');

    // 2. Création dossier pro si absent
    await _exec('mkdir -p /userdata/system/pro');

    // 3. Téléchargement du script et des keys
    _appendLog('\n⬇️ Téléchargement du script foclabroc-tools...');
    ...
    _appendLog('\n✅ Foclabroc Toolbox installé dans Ports !');
    _appendLog('🔁 Redémarrage d\'EmulationStation...');
      ...
      await showDialog(
        ...
        builder: (ctx) => AlertDialog(
          ...
          title: const Row(children: [
            Icon(Icons.check_circle_rounded, color: Color(0xFF50FA7B), size: 22),
            SizedBox(width: 10),
            Flexible(child: Text('Toolbox installée !')),
          ]),
          content: const Text(
            'Foclabroc Toolbox est disponible dans le menu Ports.\nEmulationStation va redémarrer.',
            style: TextStyle(color: Colors.white70, fontSize: 12),
          ),
          actions: [
            ElevatedButton(onPressed: () => Navigator.pop(ctx), child: const Text('OK')),
              ...
              child: Card(
                child: Padding(
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    const SizedBox(height: 10),
                    const Text(
                      'Installe la Foclabroc Toolbox directement dans le menu Ports de Batocera pour y accéder depuis l\'interface principale.',
                      style: TextStyle(color: Colors.white54, fontSize: 12, height: 1.5),
                    ),
                    const SizedBox(height: 8),
                    Wrap(spacing: 6, runSpacing: 6, children: [
                    ...
                    Container(
                      ...
                      child: const Row(children: [
                        ...
                        SizedBox(width: 6),
                        Text('EmulationStation sera relancé à la fin',
                            style: TextStyle(color: Colors.amberAccent, fontSize: 11)),
                      ]),
                    ),
                    const SizedBox(height: 20),
                    SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        ...
                        icon: _running
                            ...
                            : const Icon(Icons.download_rounded),
                        label: Text(_running ? 'Installation en cours...' : 'Installer dans Ports'),
                      ),
                    ),
                  ]),
                ),
...
class _RgsxInstallScreenState extends State<_RgsxInstallScreen> {
  ...
  Future<void> _launch() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Row(children: [
          ...
          SizedBox(width: 10),
          Flexible(child: Text('Installer RGSX ?', overflow: TextOverflow.ellipsis)),
        ]),
        content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text(
            'Télécharge et installe le pack RetroGameSets (RGSX) dans le menu Ports.',
            style: TextStyle(color: Colors.white70, fontSize: 12, height: 1.5),
          ),
          const SizedBox(height: 10),
          Container(
            ...
            child: const Row(children: [
              ...
              SizedBox(width: 6),
              Expanded(child: Text(
                'Requiert python3. Fichier volumineux.',
    ...
    // 1. Vérifie python3
    _appendLog('🔍 Vérification de python3...');
    final py = await _exec('command -v python3 && echo ok || echo fail');
    if (!py.contains('ok')) {
      _appendLog('❌ python3 non trouvé sur Batocera.');
      setState(() => _running = false);
      return;
    }
    _appendLog('✅ python3 disponible.');

    // 2. Crée les dossiers
    await _exec('mkdir -p "$_portsDir" "$_logDir"');

    // 3. Récupère l'URL du dernier release GitHub
    _appendLog('\n🔍 Récupération de l\'URL du dernier release...');
    ...
    _appendLog('\n⬇️ Téléchargement de RGSX...');
    _appendLog('  (fichier volumineux, patience)');
    ...
    if (!zipOk.contains('ok')) {
      _appendLog('❌ ZIP corrompu.');
      await _exec('rm -f /tmp/_rgsx.zip');
      setState(() => _running = false);
      return;
    }
    _appendLog('✅ ZIP valide.');

    // 6. Supprime l'ancienne installation
    _appendLog('\n🗑️ Suppression ancienne installation...');
    ...
    await _exec('rm -f /tmp/_rgsx.zip');
    // Trouve le dossier RGSX extrait (nom peut varier)
    final extractedDir = (await _exec('ls "$_portsDir/" | grep -i rgsx | head -1')).trim();
    _appendLog('  📁 Dossier : $extractedDir');
    if (extractedDir.isEmpty) {
      _appendLog('❌ Répertoire RGSX non trouvé après extraction.');
      setState(() => _running = false);
      return;
    }
      ...
      await showDialog(
        ...
        builder: (ctx) => AlertDialog(
          ...
          title: const Row(children: [
            Icon(Icons.check_circle_rounded, color: Color(0xFF50FA7B), size: 22),
            SizedBox(width: 10),
            Flexible(child: Text('RGSX installé !')),
          ]),
          content: const Text(
            'RGSX est disponible dans le menu Ports.\nMettez à jour la liste des jeux si nécessaire.',
            style: TextStyle(color: Colors.white70, fontSize: 12),
          ),
          actions: [
            ElevatedButton(onPressed: () => Navigator.pop(ctx), child: const Text('OK')),
                      ...
                      child: ElevatedButton.icon(
                        ...
                        icon: _running
                            ? const SizedBox(width: 16, height: 16,
                                child: CircularProgressIndicator(strokeWidth: 2, color: Colors.black))
                            : const Icon(Icons.download_rounded),
                        label: Text(_running ? 'Installation en cours...' : 'Installer RGSX'),
                      ),
                    ),
                  ]),
                ),
