                        ...
                        _loading
                            ...
                            : Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                ...
                                if (_bestName.isNotEmpty)
                                  Text('by $_bestName', style: const TextStyle(color: Colors.white38, fontSize: 11)),
                              ]),
                      ]),
                    ),
                    if (_bestScore > 0)
...
class _BreakoutGameState extends State<_BreakoutGame> with TickerProviderStateMixin {
  ...
  List<List<bool>> _buildShape(int cols, int rows) {
    // All shapes designed to be symmetric on 6 columns
    final List<bool Function(int, int)> shapes = [
      // Perfect checkerboard
      (c, r) => (c + r) % 2 == 0,
      // V shape (two symmetric diagonals)
      (c, r) => (c <= cols ~/ 2)
          ? r == c
          : r == cols - 1 - c,
      // Centered diamond (symmetric)
      (c, r) {
        final cx = (cols - 1) / 2.0;
        final cy = (rows - 1) / 2.0;
        return ((c - cx).abs() + (r - cy).abs()) <= (cols / 2.0).floor();
      },
      // Centered cross (columns 2-3 + center rows)
  ...
  void _buildLevel() {
    ...
    // Dynamic calculation: bricks adapted to screen width with 8px margin
    const marginX = 4.0;
          ...
          child: Column(children: [
            ...
            Container(
              ...
              child: Column(children: [
                ...
                Text('Level $_level · ${_lives > 0 ? "$_lives vie(s)" : "0 vie"}',
